...
// HuggingFace base for the voice model + config.  en_US-amy-medium
// keeps the same speaker as the original amy-low pick (so users get a
// consistent voice) but at the higher-quality medium tier, which has
// cleaner fricative rendering.  Earlier we tried lessac-medium — that
// voice has the cleanest fricatives but the user reported solfege
// syllables stopped auto-loading after the swap (2026-05-05),
// suggesting a worker/config compatibility issue with that model.
// amy-medium addresses the original "lisp" complaint without that
// regression: fixes the muddy /s/ /θ/ /f/ phonemes that low-quality
// compression had blurred, while keeping the same speaker the
// piperSpeech worker was already known to drive cleanly.
const PIPER_PUBLIC_BASE = "/piper";
const VOICE_MODEL_URL = "https://huggingface.co/rhasspy/piper-voices/resolve/v1.0.0/en/en_US/amy/medium/en_US-amy-medium.onnx";
const VOICE_CONFIG_URL = "https://huggingface.co/rhasspy/piper-voices/resolve/v1.0.0/en/en_US/amy/medium/en_US-amy-medium.onnx.json";
