...
export const TONALITY_FAMILIES: { key: string; label: string; color: string; tonalities: string[] }[] = [
  ...
  // Mode names use the harmonic-minor / melodic-minor convention:
  // closest Greek mode + specific accidentals.  Single-letter qualifier
...
export function bankToScaleFamMode(tonality: string): [string, string] {
  ...
  const KEY_TO_FAMILY: Record<string, string> = {
    major: "Major Family",
    harmonic: "Harmonic Minor Family",
    melodic: "Melodic Minor Family",
    subminor: "Subminor Diatonic Family",
