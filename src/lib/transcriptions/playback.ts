...
const SOURCE_KIT: Record<TxSource, VoiceKit> = {
  ...
  // Blues is audio-only (real recordings) — these kits are unused, but every
...
export async function playExcerpt(ex: TxExcerpt, opts: PlayOptions): Promise<PlayHandle> {
  ...
  if ((opts.withChords || opts.withBass) && ex.chords.length) {
    // Rootless jazz voicings only when a bass line will supply the root;
    // otherwise the comp includes the root so the harmony is grounded.
