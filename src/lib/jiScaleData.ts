...
const JI_SCALES: JiScaleSpec[] = [
  ...
  // True Mohajira: generated by stacking the 11/9 neutral 3rd, gives
  // a 7-note scale with a #4 (11/8 super-4th) — the canonical
  // pure-11-limit diatonic.  Distinct from Maqam Rast (which has a
  // perfect 4th); the scales were accidentally identical in an
  // earlier revision.  Cents corrected per Mohajira's 11/9 generator
  // chain: [1, 9/8, 11/9, 11/8, 3/2, 27/16, 11/6].
  { name: "Mohajira",
    steps: [["1",0],["2",203.9],["b3",347.4],["#4",551.3],["5",702.0],["6",905.9],["b7",1049.4]] },
  ...
  // that make sense just like 11 limit for the 13 limit".  Same logic
  // as the 11-limit maqam additions: build scales out of 4/3-bounded
  // tetrachords using 13-limit ratios so each tetrachord closes
  // cleanly under its constituent intervals.  53-EDO has very accurate
  // 13-limit cells (13/8 ≈ step 37, 13/12 ≈ step 6, 39/32 ≈ step 15,
  // 13/10 ≈ step 20).  These scales are exposed only for 53-EDO since
  // 41-EDO collapses several of the relevant cells (13/12 and 12/11
  // both round to step 5 in 41-EDO).
  //
  // Beirut jins: 1 - 13/12 (139¢) - 5/4 (386¢) - 4/3 — sub-neutral 2nd + JI M3
  // Tridecimal Hijaz jins: 1 - 14/13 (128¢) - 16/13 (359¢) - 4/3 — narrow b2 + 13-limit n3
  { name: "Tridecimal Major",
    // 1 - 9/8 - 13/10 (wide M3) - 4/3 - 3/2 - 13/8 (wide M6) - 27/14
    // Distinct from Diatonic Major: thirds and sixths are notably
    // wide (13-limit "supermajor" cousin).  All 6/7-prime ratios at
    // 3rd / 6th / 7th — fully 13-limit at the colour positions.
    steps: [["1",0],["2",203.9],["3",454.2],["4",498.0],["5",702.0],["6",840.5],["7",1137.0]] },
  { name: "Tridecimal Minor",
    // 1 - 9/8 - 13/11 (wide m3) - 4/3 - 3/2 - 13/8 - 13/7
    // 13-limit minor: b3 sits between Pyth m3 (294¢) and Cm3 (316¢).
    steps: [["1",0],["2",203.9],["b3",289.2],["4",498.0],["5",702.0],["b6",840.5],["b7",1071.7]] },
  { name: "Beirut",
    // Beirut tetrachord on 1 + Rast jins on 5: 1 - 13/12 - 5/4 - 4/3
    // - 3/2 - n6 - n7.  13-limit hybrid — sub-neutral 2nd combined
    // with classic JI major 3rd makes this distinctly Lebanese-flavoured.
    steps: [["1",0],["b2",138.6],["3",386.3],["4",498.0],["5",702.0],["b6",852.6],["b7",1049.4]] },
  { name: "Tridecimal Hijaz",
    // Tridecimal Hijaz: narrower b2 (14/13 = 128¢) + 13-limit n3
    // (16/13 = 359¢ — close to a sub-neutral 3rd).  Provides a 13-limit
    // alternative to standard Hijaz's 12/11 + 5/4 pairing.
    steps: [["1",0],["b2",128.3],["b3",359.5],["4",498.0],["5",702.0],["b6",840.5],["b7",1071.7]] },
...
/** 53-EDO curated set per direct user direction (2026-05-05/06).
 *  Restricted to 3-limit (Pyth Major / Harmonic Minor), 5-limit
 *  (Classic Major / Minor / Classic Harmonic Minor M7), and 7-limit
 *  (Supermajor / Subminor / Subharmonic Minor M7).  The 13/17-limit
 *  Supraminor / Supraharmonic Minor M7 entries have been removed —
 *  they grafted high-limit thirds onto a 3-limit (Pyth M2/P4/P5)
 *  backbone, which forces the chord stacks to use junk leftover
 *  intervals (same nonsense as the now-removed Diatonic Neutral).
 *  Higher-limit content (11-limit, 13-limit) lives in the maqam +
 *  tridecimal-tetrachord families instead. */
...
// 13-limit tridecimal-tetrachord scales — exposed only for 53-EDO per
// direct user direction (2026-05-06): 53-EDO has clean 13-limit cells
// for 13/8 / 13/12 / 13/10 / 13/11 that 41-EDO collapses, so these
// scales actually sound 13-limit there.
const TRIDECIMAL_TONALITIES = [
  "Tridecimal Major",
  "Tridecimal Minor",
  "Beirut",
  "Tridecimal Hijaz",
];
FIFTY_THREE_EDO_TONALITY_FAMILIES.push({ parent: "Tridecimal", tonalities: TRIDECIMAL_TONALITIES });
