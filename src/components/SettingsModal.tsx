...
interface Props {
  ...
  showMetronomeTimer: boolean;
  onShowMetronomeTimerChange: (v: boolean) => void;
}

export default function SettingsModal({ onClose, onDataImported, betaPlayRotation, onBetaPlayRotationChange, betaIntervalChain, onBetaIntervalChainChange, betaComma, onBetaCommaChange, betaTransform, onBetaTransformChange, betaMathLab, onBetaMathLabChange, betaMode, onBetaModeChange, academicMode, academicAvailable = false, onAcademicModeChange, showMetronomeTimer, onShowMetronomeTimerChange }: Props) {
    ...
    >
      <div className="bg-[#111] border border-[#2a2a2a] rounded-xl w-full max-w-md shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="px-5 py-5 space-y-6 overflow-y-auto">
          ...
          {/* Tools / surfaces — Metronome + Timer strip toggle.
              Hidden by default per direct user direction (2026-05-06):
              "remove metornome and timer from everything unless i
              toggle it on in settings". */}
          <div>
            <h3 className="text-xs font-semibold text-[#60c0a0] uppercase tracking-widest mb-3">Tools</h3>
            <div className="space-y-2">
              <label className="flex items-center gap-3 px-3 py-2.5 bg-[#1a1a1a] border border-[#2a2a2a] hover:border-[#3a3a3a] rounded-lg cursor-pointer transition-colors">
                <input
                  type="checkbox"
                  checked={showMetronomeTimer}
                  onChange={e => onShowMetronomeTimerChange(e.target.checked)}
                  className="accent-[#60c0a0] w-4 h-4 cursor-pointer"
                />
                <div className="flex-1">
                  <div className="text-sm font-medium text-[#ccc]">Metronome &amp; Timer</div>
                  <div className="text-xs text-[#555]">Show the metronome + countdown timer strip in the page header.  Off by default; turn on if you want to time your practice or click against a pulse.</div>
                </div>
              </label>
            </div>
          </div>
