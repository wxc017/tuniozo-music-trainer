...
/*
 * Standard tuning open-string pitches relative to pitch 0 (C4).
 * E2=-20, A2=-15, D3=-10, G3=-5, B3=-1, E4=4
 */
/* High to low (top to bottom), matching standard fretboard diagrams.
 * Thinnest string (high E) at top, thickest (low E) at bottom. */
const OPEN_STRINGS = [4, -1, -5, -10, -15, -20];
...
export default function GuitarFretboard({
  ...
  const { dots, strings, fretLines, fretMarkers, viewW, viewH } = useMemo(() => {
    ...
    const PAD_T = 16;
    const PAD_B = 16;
    const FRET_W = 36;  // width per fret
    const STRING_GAP = 22;
    ...
    for (const f of markerFrets) {
      ...
      if (doubleDotFrets.has(f)) {
        markers.push({ x: cx, y: PAD_T + 1 * STRING_GAP });
        markers.push({ x: cx, y: PAD_T + (numStrings - 2) * STRING_GAP });
      } else {
      ...
      >
        ...
        {strings.map((s, i) => (
          <line key={`s${i}`} x1={s.x1} y1={s.y} x2={s.x2} y2={s.y}
            stroke="#777" strokeWidth={1 + i * 0.3} />
        ))}
